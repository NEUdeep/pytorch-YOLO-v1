...
def parse_args():
    ...
    parser.add_argument('--dir_path', type=str, default='/workspace/mnt/storage/kanghaidong/khdwork/work_data/detection_data/public-data/')
...
if __name__ == '__main__':
    ...
    save_path = '/workspace/mnt/storage/kanghaidong/action_detect_data/pytorch-YOLO-v1/'

    for data_ in sets:

        if not os.path.exists(save_path + 'VOC%s/Label/' % (data_[0])):
            os.makedirs(save_path + 'VOC%s/Label/' % (data_[0]))

        name_list = open(root_dir + 'VOC%s/ImageSets/Main/%s.txt' % (data_[0], data_[1])).read().strip().split()

        print(len(name_list))
        name_list = tqdm(name_list)
        data_list = open(save_path + 'VOC%s_%s.txt' % (data_[0], data_[1]), 'w')

        file_writer = ''
        for i, xml_name in enumerate(name_list):
            file_path = root_dir + 'VOC%s/Annotations/%s.xml' % (data_[0], xml_name)
            label_file = save_path + 'VOC%s/Label/%s.txt' % (data_[0], xml_name)
